...
"#)?;
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_mmm"))
        .current_dir(&temp_dir)
        .args(["improve", "--config", custom_config.to_str().unwrap(), "--max-iterations", "0"])
        .output()?;
    ...
    assert!(
        ...
        "Should attempt to run custom command. STDOUT: {}, STDERR: {}",
        stdout,
        stderr
    );
...
"#)?;
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_mmm"))
        .current_dir(&temp_dir)
        .args(["improve", "--config", yaml_config.to_str().unwrap(), "--max-iterations", "0"])
        .output()?;
    ...
    assert!(
        ...
        stderr.contains("Claude CLI not found") ||
        stderr.contains("claude"),
        "Should attempt to run YAML command. STDOUT: {}, STDERR: {}",
        stdout,
        stderr
    );
...
fn test_improve_with_default_config() -> anyhow::Result<()> {
    ...
    fs::write(mmm_dir.join("config.toml"), r#"
...
"#)?;
    ...
    assert!(
        ...
        stderr.contains("Claude CLI not found") ||
        stderr.contains("claude"),
        "Should use default config. STDOUT: {}, STDERR: {}",
        stdout,
        stderr
    );
...
fn test_improve_with_legacy_workflow_toml() -> anyhow::Result<()> {
    ...
    let mmm_dir = temp_dir.path().join(".mmm");
    fs::create_dir_all(&mmm_dir)?;
    fs::write(mmm_dir.join("workflow.toml"), r#"
...
"#)?;
    ...
    assert!(
        ...
        "Should warn about deprecated workflow.toml. STDERR: {}",
        stderr
    );
...
fn test_improve_with_invalid_config_path() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Should fail with missing file error. STDERR: {}",
        stderr
    );
...
fn test_improve_with_unsupported_config_format() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Should fail with unsupported format error. STDERR: {}",
        stderr
    );
