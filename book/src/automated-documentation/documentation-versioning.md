...
**For Projects Adopting Versioned Documentation:**

1. **Start with Current Version**: Deploy your current version first as both `/vX.Y.Z/` and `/latest/` before adding historical versions

2. **Use Semantic Versioning**: Tag releases with semver format (`vX.Y.Z`) for automatic version sorting

3. **Test Before Tagging**: Run the book workflow locally before pushing version tags to catch documentation issues

4. **Document Breaking Changes**: When releasing new major versions, clearly document breaking changes in the documentation

5. **Regular Cleanup**: Review deployed versions quarterly and remove very old versions per retention policy

6. **Monitor Storage**: Keep an eye on gh-pages branch size (GitHub Pages has 1GB soft limit)

7. **Version-Specific Examples**: When updating documentation for new features, add version notes indicating when features were introduced

**Source**: specs/155-versioned-documentation-deployment.md:393-402
...
- Verify `version-selector.js` and `.css` are in `book/theme/` directory
- Check `book.toml` includes `additional-js` and `additional-css` entries
- Ensure `versions.json` exists at documentation root
- Check browser console for JavaScript errors

**Broken Version Links:**
- Verify version paths in `versions.json` match actual deployed directories
- Ensure all versions use consistent URL structure
- Check that `/latest/` is updated after deploying new versions

**Deployment Failures:**
- Verify git tag follows semver format (`vX.Y.Z`)
- Check GitHub Actions logs for build errors
- Ensure book workflow completes successfully for that version
- Verify `peaceiris/actions-gh-pages` action permissions

**Version Not Updating:**
- Check that `versions.json` generation script ran after deployment
- Verify `/latest/` directory was updated for newest version
- Clear browser cache to see updated version selector

**Source**: specs/155-versioned-documentation-deployment.md:410-419

### Implementation Status

**Current Status: Draft / Not Implemented**

All three specifications are currently in draft status:
- **Spec 154** (mdBook Version Selector UI): Draft - UI component design complete, not yet implemented
- **Spec 155** (Versioned Documentation Deployment): Draft - Deployment workflow design complete, not yet implemented
- **Spec 156** (Version-Aware Book Workflow): Draft - Workflow enhancements designed, not yet implemented

**Source**: specs/154-mdbook-version-selector-ui.md:6, specs/155-versioned-documentation-deployment.md:6, specs/156-version-aware-book-workflow.md:6

**What Exists Today:**
- Single-version documentation deployment (`.github/workflows/deploy-docs.yml`)
- Automated documentation workflow without version support (`workflows/book-docs-drift.yml`)
- GitHub Pages deployment to root path only

**Planned Implementation Path:**
1. **Phase 1**: Implement version selector UI component (Spec 154)
2. **Phase 2**: Create versioned deployment workflow (Spec 155)
3. **Phase 3**: Enhance book workflow with VERSION parameter (Spec 156)
4. **Phase 4**: Deploy Prodigy's own documentation with versioning
5. **Phase 5**: Document lessons learned and create templates for other projects

### Future Enhancements

**Beyond the current design, potential future additions include:**

- **Prodigy CLI Integration**:
  ```bash
  prodigy book add-versioning  # Automatically set up versioning
  prodigy book deploy --version v0.2.6  # Manual deployment command
  ```

- **Automatic versions.json Generation**: Parse git tags and generate `versions.json` automatically from repository metadata

- **Version Comparison View**: Side-by-side diff showing documentation changes between versions

- **Search Across Versions**: Enhanced search that can find content across all deployed versions

- **Version-Specific Changelogs**: Automatically extract and display changelog for each version in the docs

**Source**: specs/154-mdbook-version-selector-ui.md:269-273

### See Also

- [GitHub Actions Integration](github-actions-integration.md) - Current (non-versioned) deployment workflow
- [Understanding the Workflow](understanding-the-workflow.md) - How the automated documentation system works
- [Best Practices](best-practices.md) - General documentation best practices
- [Quick Start](quick-start.md) - Getting started with automated documentation
- [Advanced Configuration](advanced-configuration.md) - Customizing the documentation workflow

**Related Specifications:**
- Spec 154: mdBook Version Selector UI (specs/154-mdbook-version-selector-ui.md)
- Spec 155: Versioned Documentation Deployment (specs/155-versioned-documentation-deployment.md)
- Spec 156: Version-Aware Book Workflow (specs/156-version-aware-book-workflow.md)
