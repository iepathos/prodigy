...
use prodigy::storage::global::GlobalStorage;
use serde_json::json;
            ...
            |b, &size| {
                b.to_async(&rt).iter_batched(
                    || {
                        ...
                        let storage = GlobalStorage::new(temp_dir.path().to_path_buf());
                        let data = create_test_json(size);
                        (storage, data, temp_dir)
                    },
                    |(storage, data, _temp_dir)| async move {
                        let path = storage.state_dir("test-repo").join("test.json");
        ...
        .bench_function("create_directory_tree", |b| {
            b.to_async(&rt).iter_batched(
                ...
                |(storage, _temp_dir)| async move {
                    ...
                    let dirs = vec![
                        storage.events_dir("repo", "job1"),
                        storage.dlq_dir("repo", "job1"),
        ...
        .bench_function("list_directory_contents", |b| {
            b.to_async(&rt).iter_batched(
                || {
                    ...
                    rt_local.block_on(async {
                        let dir = storage.state_dir("repo");
                        tokio::fs::create_dir_all(&dir).await.unwrap();
                        for i in 0..20 {
