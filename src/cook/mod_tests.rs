...
use super::*;
use crate::abstractions::{MockClaudeClient, MockGitOperations, GitOperations, ClaudeClient};
...
mod core_tests {
    ...
    fn create_session_state(session_id: &str, focus: Option<&str>, max_iterations: usize, worktree_mode: bool) -> TestSessionState {
        TestSessionState {
    ...
    async fn handle_worktree_merge(_worktree_mgr: &WorktreeManager, _worktree_name: &str, _auto_accept: bool) -> Result<()> {
        // Placeholder
        Ok(())
    }

    fn format_session_summary(state: &TestSessionState) -> String {
        let mode = if state.worktree_mode { "worktree" } else { "direct" };
    ...
    async fn save_checkpoint(_project_path: &Path, _state: &TestSessionState, _spec_id: Option<&str>) {
        // Placeholder
    }
...
"#;
        ...
        tokio::fs::write(&playbook_path, yaml_content).await.unwrap();
    ...
    async fn test_load_playbook_json() {
        ...
        tokio::fs::write(&playbook_path, json_content).await.unwrap();
    ...
    async fn test_load_playbook_invalid_format() {
        ...
        // Invalid YAML
        tokio::fs::write(&playbook_path, "invalid: [yaml content").await.unwrap();
...
"#,
    ...
    async fn test_extract_spec_from_git() {
        ...
        git_ops.add_success_response("add: spec iteration-12345-improvements").await;
        
        let result = extract_spec_from_git(&git_ops).await;
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), Some("iteration-12345-improvements".to_string()));
    }
    ...
    async fn test_extract_spec_from_git_error() {
        let git_ops = MockGitOperations::new();
        git_ops.add_error_response("fatal: not a git repository").await;
    ...
    async fn test_check_for_changes() {
        ...
        // Test with changes
        git_ops.add_success_response("M src/main.rs\nA src/lib.rs").await;
