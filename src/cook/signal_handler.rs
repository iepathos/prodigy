...
mod tests {
    ...
    fn create_test_session_state(manager: &WorktreeManager, session_name: &str) -> Result<()> {
        ...
        let state = WorktreeState {
            ...
            branch: format!("mmm-{}", session_name),
            created_at: Utc::now(),
            updated_at: Utc::now(),
        ...
        let state_file = metadata_dir.join(format!("{}.json", session_name));
    ...
    fn test_update_interrupted_state() {
        ...
        let state_file = worktree_manager.base_dir.join(".metadata").join(format!("{}.json", session_name));
