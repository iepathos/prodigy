...
mod enhanced_progress_tracker_tests {
    ...
    async fn test_update_agent_progress() {
        ...
        tracker.update_agent_progress("agent-1", progress.clone()).await.unwrap();
    ...
    async fn test_mark_item_completed() {
        ...
        tracker.update_agent_progress("agent-1", progress).await.unwrap();
    ...
    async fn test_mark_item_failed() {
        ...
        tracker.update_agent_progress("agent-1", progress).await.unwrap();
        tracker.mark_item_failed("agent-1", "Test error".to_string()).await.unwrap();
    ...
    async fn test_overall_progress_calculation() {
        ...
        for i in 0..3 {
            ...
            tracker.update_agent_progress(&agent_id, progress).await.unwrap();
        ...
        tracker.update_agent_progress("agent-fail", progress).await.unwrap();
        tracker.mark_item_failed("agent-fail", "Error".to_string()).await.unwrap();
    ...
    async fn test_create_snapshot() {
        ...
        for i in 0..3 {
            ...
            tracker.update_agent_progress(&format!("agent-{}", i), progress).await.unwrap();
        }
    ...
    async fn test_metrics_recalculation() {
        ...
        for i in 0..3 {
            tracker.mark_item_completed(&format!("agent-{}", i)).await.unwrap();
        }
    ...
    async fn test_agent_state_transitions() {
        ...
        // Start as queued
        tracker.update_agent_state(agent_id, AgentState::Queued).await.unwrap();
        {
            let agents = tracker.agents.read().await;
            assert!(matches!(agents.get(agent_id), Some(agent) if matches!(agent.state, AgentState::Queued)));
        }
        
        // Move to initializing
        tracker.update_agent_state(agent_id, AgentState::Initializing).await.unwrap();
        {
            let agents = tracker.agents.read().await;
            assert!(matches!(agents.get(agent_id), Some(agent) if matches!(agent.state, AgentState::Initializing)));
        }
        
        // Move to running
        tracker.update_agent_state(agent_id, AgentState::Running {
            step: "Processing".to_string(),
            progress: 25.0,
        }).await.unwrap();
        {
        ...
        // Move to retrying
        tracker.update_agent_state(agent_id, AgentState::Retrying { attempt: 1 }).await.unwrap();
        {
            let agents = tracker.agents.read().await;
            assert!(matches!(
                agents.get(agent_id),
        ...
        tracker.update_agent_state(agent_id, AgentState::DeadLettered).await.unwrap();
        {
            let agents = tracker.agents.read().await;
            assert!(matches!(
                agents.get(agent_id),
...
mod integration_tests {
    ...
    async fn test_complete_workflow_simulation() {
        ...
        for i in 0..10 {
            ...
            if i % 5 == 0 && i > 0 {
                tracker.mark_item_failed(&agent_id, format!("Error in item {}", i)).await.unwrap();
            } else {
                tracker.mark_item_completed(&agent_id).await.unwrap();
            }
        }
    ...
    async fn test_concurrent_agent_updates() {
        let tracker = Arc::new(EnhancedProgressTracker::new("concurrent-test".to_string(), 100));
        ...
        for i in 0..20 {
            ...
            let handle = tokio::spawn(async move {
                ...
                for step in 1..=5 {
                    ...
                    tracker_clone.update_agent_progress(&agent_id, progress).await.unwrap();
