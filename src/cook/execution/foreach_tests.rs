...
mod foreach_execution_tests {
    ...
    async fn test_foreach_parallel_execution() {
        ...
        let config = ForeachConfig {
            ...
            do_block: vec![Box::new(WorkflowStepCommand::Shell {
                // This command will help us test parallelism
                command: "sleep 0.1 && echo Processing ${item}".to_string(),
                capture_output: None,
                env: None,
                fail_workflow: false,
                description: None,
    ...
    async fn test_foreach_continue_on_error() {
        let config = ForeachConfig {
            ...
            do_block: vec![Box::new(WorkflowStepCommand::Shell {
                ...
                command: "test \"${item}\" != \"fail\" && echo Success || exit 1".to_string(),
                capture_output: None,
                env: None,
                fail_workflow: false,
                description: None,
    ...
    async fn test_foreach_variable_interpolation() {
        let config = ForeachConfig {
            ...
            parallel: ParallelConfig::Boolean(false),
            do_block: vec![Box::new(WorkflowStepCommand::Shell {
                command: "echo 'Processing ${item} (${index}/${total})'".to_string(),
                capture_output: None,
                env: None,
                fail_workflow: false,
                description: None,
    ...
    async fn test_foreach_multiple_commands() {
        let config = ForeachConfig {
            ...
            parallel: ParallelConfig::Boolean(false),
            do_block: vec![
                Box::new(WorkflowStepCommand::Shell {
                    command: "echo 'Starting ${item}'".to_string(),
                    capture_output: None,
                    env: None,
                    fail_workflow: false,
                    description: None,
                ...
                Box::new(WorkflowStepCommand::Shell {
                    command: "echo 'Processing ${item}'".to_string(),
                    capture_output: None,
                    env: None,
                    fail_workflow: false,
                    description: None,
                ...
                Box::new(WorkflowStepCommand::Shell {
                    command: "echo 'Finished ${item}'".to_string(),
                    capture_output: None,
                    env: None,
                    fail_workflow: false,
                    description: None,
