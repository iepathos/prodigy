...
async fn test_capture_with_multiline_handling() {
    ...
    assert_eq!(
        engine.get_variable_value("JOINED").unwrap(),
        &json!("line1 line2 line3")
    );
    assert_eq!(
        engine.get_variable_value("ARRAY").unwrap(),
        &json!(["line1", "line2", "line3"])
    );
}
...
async fn test_multiple_captures_from_different_commands() {
    ...
    assert_eq!(
        engine.get_variable_value("CMD0_OUTPUT").unwrap(),
        &json!("output0")
    );
    assert_eq!(
        engine.get_variable_value("CMD1_OUTPUT").unwrap(),
        &json!("output1")
    );
    assert_eq!(
        engine.get_variable_value("CMD2_OUTPUT").unwrap(),
        &json!("output2")
    );
}
