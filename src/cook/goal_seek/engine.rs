...
impl GoalSeekEngine {
    ...
    pub async fn seek(&mut self, config: GoalSeekConfig) -> Result<GoalSeekResult> {
        ...
        loop {
            ...
            let command = config
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!("Goal seek must have either 'claude' or 'shell' command")
                })?;

            debug!("Executing: {}", command);
            let attempt_result = self
                .execute_command_with_context(&command, attempt > 1)
                .await?;
            ...
            info!(
                "📊 Score: {}/100 (threshold: {})",
                validation.score, config.threshold
            );
        ...
        if has_validation_context && !self.attempt_history.is_empty() {
            ...
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&last_attempt.output) {
                if let Some(gaps) = json.get("gaps") {
                    context
                        .env_vars
                        .insert("PRODIGY_VALIDATION_GAPS".to_string(), gaps.to_string());
                }
            }
        }
    ...
    async fn validate_attempt(
        ...
        let validation_output = self
            .execute_command_with_context(&config.validate, false)
            .await?;
