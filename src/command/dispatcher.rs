use super::{Command, CommandContext, CommandOutput, CommandRegistry, CommandHistory, BatchCommand};
...
impl CommandDispatcher {
    ...
    pub async fn dispatch(&mut self, command_line: &str, context: CommandContext) -> Result<CommandOutput> {
        ...
        self.history.add_result(command_line.to_string(), &output).await?;
        
        Ok(output)
    }
    
    pub async fn dispatch_batch(&mut self, batch: BatchCommand, context: CommandContext) -> Result<Vec<CommandOutput>> {
    ...
    pub async fn replay_command(&mut self, index: usize, context: CommandContext) -> Result<CommandOutput> {
        let command = self.history.get_command(index).await?
