use super::{Checkpoint, ProjectState, init_database};
...
impl StateManager {
    ...
    pub async fn restore_checkpoint(&self, checkpoint_id: &str) -> Result<()> {
        ...
        let checkpoint = current_state.checkpoints
            .iter()
    ...
    pub async fn get_spec_executions(&self, spec_id: &str) -> Result<Vec<crate::spec::SpecExecution>> {
        // First get the spec database ID
        let spec_row = sqlx::query(
            "SELECT id FROM specifications WHERE name = ?1 AND project_id = ?2"
        )
        .bind(spec_id)
        .bind(self.project_id)
        .fetch_optional(&self.pool)
