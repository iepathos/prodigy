...
impl Default for AnalysisConfig {
    fn default() -> Self {
        Self {
            ...
            run_context: true,
            verbose: false,
            run_coverage: true,  // Default to true for accurate coverage
        }
    }
}
...
fn commit_all_analysis(
    ...
    let commit_msg = if let (Some(ctx), Some(m)) = (context, metrics) {
        format!(
            ...
            health_score.overall,
            if m.test_coverage == 0.0 { "N/A".to_string() } else { format!("{:.1}%", m.test_coverage) },
