...
impl Achievement {
    ...
    pub fn display_progress(&self) {
        ...
        let bar_width = 20;
        let filled = (percentage * bar_width / 100) as usize;
        let empty = bar_width - filled;
        
        println!("{} Progress towards next achievement:",
            "🎯".bold()
        );
        println!("   {} - {}/{} {}",
            self.name.cyan(),
...
impl Streak {
    ...
    pub fn display(&self) {
        if self.current > 1 {
            println!("{} {} day improvement streak!",
                "🔥".bold(),
                self.current.to_string().yellow().bold()
            );
            
            if self.current == self.longest {
                println!("   {} This is your longest streak!",
                    "⭐".yellow()
                );
            } else {
                println!("   Your longest streak: {} days",
...
impl SuccessMessage {
    ...
    pub fn contextual(improvement_type: &str, impact: f32) -> String {
        match improvement_type {
            ...
            "performance" => {
                "⚡ Speed improvements applied! Your code is now blazing fast."
            }
            _ => Self::random(),
        }.to_string()
    }
...
impl Leaderboard {
    ...
    pub fn update(&mut self, name: String, score: f32, daily_improvement: f32) {
        ...
        self.members.sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap());
    ...
    pub fn display(&self) {
        ...
        for member in self.members.iter().take(5) {
            ...
            let star = if highlight && member.daily_improvement > 0.0 { " ⭐" } else { "" };
            
            let line = format!("{}. {} {}:  {:.1}/10 ({}{})",
                member.rank,
                medal,
                member.name,
                member.score,
                if member.daily_improvement > 0.0 { "↑" } else { "→" },
        ...
        if let Some(user) = self.members.iter().find(|m| m.name == self.current_user) {
            let max_improvement = self.members
                .iter()
            ...
            if user.daily_improvement == max_improvement && max_improvement > 0.0 {
                println!();
                println!("{}", "You had the biggest improvement today! 🎉".green().bold());
            }
        }
    }
}