...
impl SmartHelper {
    ...
    pub fn update_context(&mut self, language: String, framework: Option<String>, test_coverage: f32) {
    ...
    pub fn suggest_next_action(&self) -> NextAction {
        match self.project_context.last_improvement {
            ...
            Some(ImprovementType::Documentation) => NextAction {
                ...
                description: "Documentation updated. Ready for performance optimization?".to_string(),
    ...
    pub fn get_suggestions(&self) -> Vec<NextAction> {
        ...
        if self.project_context.test_coverage < 60.0 {
            suggestions.push(NextAction {
                command: "mmm improve --focus tests".to_string(),
                description: format!("Test coverage is only {:.0}%. Let's improve that!", 
        ...
        if self.usage_history.iter().filter(|&&t| t == ImprovementType::Performance).count() == 0 {
            suggestions.push(NextAction {
                command: "mmm improve --focus performance".to_string(),
    ...
    pub fn display_suggestions(&self) {
        ...
        println!("{} Based on your project, consider these improvements:", "🎯".bold());
        println!();
...
impl ContextualHelp {
    ...
    pub fn new() -> Self {
        ...
        // First run help
        help_items.insert("first_run".to_string(), HelpItem {
            title: "Getting Started with MMM".to_string(),
            content: "MMM analyzes your project and improves code quality automatically. \
                     It's completely safe - all changes are validated before applying.".to_string(),
        ...
        // Error handling help
        help_items.insert("error_handling".to_string(), HelpItem {
            title: "Improving Error Handling".to_string(),
            content: "MMM will replace unwrap() calls with proper error propagation, \
                     add context to errors, and ensure all errors are handled gracefully.".to_string(),
            related_commands: vec![
...
impl LearningSystem {
    ...
    pub fn top_patterns(&self, limit: usize) -> Vec<(String, f32)> {
        let mut patterns: Vec<_> = self.success_patterns
            .iter()
            .map(|(k, v)| (k.clone(), *v))
            .collect();
