...
impl IncrementalProcessor {
    ...
    pub async fn get_changed_files(&self, project_root: &Path) -> Result<Vec<PathBuf>> {
        ...
        if changed.is_empty() {
            println!(
                "{} No changes since last run - your code is still great!",
                "✨".green()
            );
        } else {
            println!(
                "{} Found {} changed files to improve",
                "📝".cyan(),
...
impl ChangeDetector {
    ...
    async fn detect_changes(&self, root: &Path, cache: &LastRunCache) -> Result<Vec<PathBuf>> {
        // Use git if available for faster detection
        let mut changed_files = if self.has_git(root).await {
            self.git_changed_files(root, cache.timestamp).await?
        } else {
            // Fall back to filesystem scanning
            self.scan_changed_files(root, cache).await?
        };
    ...
    async fn git_changed_files(&self, root: &Path, since: SystemTime) -> Result<Vec<PathBuf>> {
        use tokio::process::Command;

        let since_timestamp = since.duration_since(SystemTime::UNIX_EPOCH)?.as_secs();
    ...
    async fn scan_changed_files(&self, root: &Path, cache: &LastRunCache) -> Result<Vec<PathBuf>> {
        use tokio::fs;
...
impl ParallelExecutor {
        ...
        Fut: std::future::Future<Output = Result<()>> + Send,
    {
        ...
        let total = tasks.len();
        let progress =
            crate::developer_experience::progress::progress_bar(total as u64, progress_message);
