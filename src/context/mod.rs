...
fn load_test_coverage(context_dir: &Path) -> Result<Option<TestCoverageMap>> {
    ...
    if coverage_summary_path.exists() {
        ...
        match serde_json::from_str::<summary::TestCoverageSummary>(&content) {
            Ok(summary) => {
                eprintln!(
            ...
            Err(_) => {
                // Deserialization failed, skip this format
            }
        }
    }
...
mod tests {
    ...
    async fn test_save_analysis_success() -> Result<()> {
        ...
        // Create a minimal project structure for analysis
        ...
        let result = save_analysis(project_path, &analysis);
        assert!(result.is_ok());
    ...
    async fn test_save_analysis_with_invalid_path() -> Result<()> {
        ...
        let result = save_analysis(invalid_path, &analysis);
    ...
    #[test]
    fn test_load_test_coverage_full_map() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let context_dir = temp_dir.path();

        // Create a full test coverage map
        ...
        std::fs::write(&coverage_path, serde_json::to_string_pretty(&coverage_map)?)?;

        // Load and verify
        let loaded = load_test_coverage(context_dir)?;
        assert!(loaded.is_some());
        let loaded_map = loaded.unwrap();
        assert_eq!(loaded_map.overall_coverage, 0.85);
        assert_eq!(loaded_map.file_coverage.len(), 1);
        assert!(loaded_map
            .file_coverage
    ...
    #[test]
    fn test_load_test_coverage_summary_only() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let context_dir = temp_dir.path();

        // Create a test coverage summary
        let summary = serde_json::json!({
            ...
            "untested_summary": {
                ...
                "by_file": [
                    {"file": "src/lib.rs", "count": 5, "highest_criticality": "Medium"}
                ]
            },
            "critical_gaps": []
        });
        ...
        // Save the summary
        let summary_path = context_dir.join("test_coverage_summary.json");
        ...
        assert!(loaded.is_some());
        let loaded_map = loaded.unwrap();
        assert_eq!(loaded_map.overall_coverage, 0.73);
        assert_eq!(loaded_map.file_coverage.len(), 1);
        assert!(loaded_map
            .file_coverage
        ...
        let file_cov = &loaded_map.file_coverage[&PathBuf::from("src/lib.rs")];
        assert_eq!(file_cov.coverage_percentage, 0.73);
        assert_eq!(file_cov.total_functions, 5); // Approximated from untested_count
        assert!(file_cov.has_tests);

        Ok(())
    }

    #[test]
    fn test_load_test_coverage_no_files() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let context_dir = temp_dir.path();

        // No coverage files exist
        let loaded = load_test_coverage(context_dir)?;
    ...
    #[test]
    fn test_load_test_coverage_invalid_json() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let context_dir = temp_dir.path();

        // Create invalid JSON in coverage file
        let coverage_path = context_dir.join("test_coverage.json");
        std::fs::write(&coverage_path, "{ invalid json content")?;

        // Should fallback to summary file
        let loaded = load_test_coverage(context_dir)?;
        assert!(loaded.is_none());
    ...
    #[test]
    fn test_load_test_coverage_prefers_full_map() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let context_dir = temp_dir.path();

        // Create both full map and summary
        let coverage_map = TestCoverageMap {
            file_coverage: {
                let mut map = HashMap::new();
                map.insert(
        ...
        std::fs::write(
            ...
            serde_json::to_string_pretty(&coverage_map)?,
        )?;
        std::fs::write(
            context_dir.join("test_coverage_summary.json"),
            serde_json::to_string_pretty(&summary)?,
        )?;

        // Should prefer full map over summary
        let loaded = load_test_coverage(context_dir)?;
        assert!(loaded.is_some());
        let loaded_map = loaded.unwrap();
        assert_eq!(loaded_map.overall_coverage, 0.90); // From full map, not summary
        assert!(loaded_map
            .file_coverage
            .contains_key(&PathBuf::from("src/main.rs")));
        assert!(!loaded_map
            .file_coverage
            .contains_key(&PathBuf::from("src/lib.rs"))); // Summary ignored

        Ok(())
    }

    #[test]
    fn test_load_test_coverage_summary_conversion() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let context_dir = temp_dir.path();

        // Create a complex summary with multiple files
        let summary = serde_json::json!({
            ...
            "untested_summary": {
                ...
                "by_file": [
                    {"file": "src/parser.rs", "count": 8, "highest_criticality": "Medium"},
                    {"file": "src/analyzer.rs", "count": 2, "highest_criticality": "High"},
                    {"file": "src/utils.rs", "count": 15, "highest_criticality": "Low"}
                ]
            },
            "critical_gaps": []
        });
        ...
        assert_eq!(loaded_map.overall_coverage, 0.65);
        assert_eq!(loaded_map.file_coverage.len(), 3);

        // Verify each file's conversion
        let parser_cov = &loaded_map.file_coverage[&PathBuf::from("src/parser.rs")];
        assert_eq!(parser_cov.coverage_percentage, 0.45);
        assert_eq!(parser_cov.total_functions, 8);
        assert!(parser_cov.has_tests);

        let analyzer_cov = &loaded_map.file_coverage[&PathBuf::from("src/analyzer.rs")];
        assert_eq!(analyzer_cov.coverage_percentage, 0.80);
        assert_eq!(analyzer_cov.total_functions, 2);
        assert!(analyzer_cov.has_tests);

        let utils_cov = &loaded_map.file_coverage[&PathBuf::from("src/utils.rs")];
        assert_eq!(utils_cov.coverage_percentage, 0.0);
        assert_eq!(utils_cov.total_functions, 15);
        assert!(!utils_cov.has_tests);

        Ok(())
    }

    #[test]
    fn test_load_test_coverage_malformed_summary() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let context_dir = temp_dir.path();

        // Create a summary with missing required fields
        let bad_summary = serde_json::json!({
            "file_coverage": {
                "src/main.rs": {
                    "coverage_percentage": 0.5
                    // Missing untested_count and has_tests
                }
            }
            // Missing overall_coverage
        });

        // Save the malformed summary
        std::fs::write(
            context_dir.join("test_coverage_summary.json"),
            serde_json::to_string_pretty(&bad_summary)?,
        )?;
    ...
    #[test]
    fn test_load_test_coverage_empty_summary() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let context_dir = temp_dir.path();

        // Create an empty but valid summary
        ...
        assert!(loaded.is_some());
        let loaded_map = loaded.unwrap();
        assert_eq!(loaded_map.overall_coverage, 0.0);
        assert_eq!(loaded_map.file_coverage.len(), 0);
        assert!(loaded_map.untested_functions.is_empty());
        assert!(loaded_map.critical_paths.is_empty());
