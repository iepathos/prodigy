...
impl BasicHybridCoverageAnalyzer {
    ...
    fn calculate_hybrid_score(
        ...
        (base_coverage - gap_penalty - critical_penalty).max(0.0).min(100.0)
    }
}
...
impl HybridCoverageAnalyzer for BasicHybridCoverageAnalyzer {
    async fn analyze_hybrid_coverage(
        ...
        let hybrid_score = self.calculate_hybrid_score(
            coverage_map,
            &priority_gaps,
            &critical_files,
        );
        ...
        for (file, coverage) in &coverage_map.file_coverage {
            if coverage.coverage_percentage < 50.0 {
                gaps.push(CoverageGap {
                    file: file.clone(),
                    functions: coverage_map.untested_functions
                        .iter()
                        .filter(|f| f.file == *file)
                        .map(|f| f.name.clone())
                        .collect(),
                    coverage_percentage: coverage.coverage_percentage,
                    risk: if coverage.coverage_percentage < 30.0 { "High" } else { "Medium" }.to_string(),
                });
            }
        }
...
mod tests {
    ...
    fn test_trend_calculation() {
        ...
        assert_eq!(analyzer.calculate_trend(&improving), TrendDirection::Improving);
        
        // Degrading trend
        let degrading = vec![5.0, 7.0, 10.0];
        assert_eq!(analyzer.calculate_trend(&degrading), TrendDirection::Degrading);
